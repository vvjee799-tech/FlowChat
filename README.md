...
- **轻量记忆与文件**：可以让 AI 从本机记忆中找回相关对话，也可以附带单个文本文件继续聊；记忆可随时关闭、逐条删除或全部清空。
- **权限不打包出售**：应用使用概览、最近活动、打开应用分别授权，没开启的工具不会进入模型请求。
...
- 在手机上快速试不同模型的回答风格。
- 把 TXT、Markdown、JSON、代码等小型文本文件附在消息里，让 AI 接着内容聊天。
...
- 文本附件限制为单个、最大 256 KB，不支持图片、PDF 或二进制文件。
